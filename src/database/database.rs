...
impl Database {
    ...
    pub async fn signup(&self, user: SignUp) -> Result<Result<Option<User>, String>, Error> {
        let is_duplicate_email = self.check_duplicate_email(user.email.clone()).await;
        match is_duplicate_email {
            Ok(duplicate) => {
                if duplicate {
                    return Ok(Err("Email already in use".to_string()))
                }
            },
        ...
        match is_duplicate_username {
            Ok(duplicate) => {
                if duplicate {
                    return Ok(Err("Username already taken".to_string()))
                }
            },
            Err(err) => return Err(err),
        };
        ...
        match created_user {
            Ok(user) => Ok(Ok(user.into_iter().next())),
            Err(err) => Err(err)
        }
    }

    pub async fn email_login(&self, credentials: EmailLoginIn) -> Result<Result<EmailLoginInSuccess, String>, Error> {
        let query = format!("SELECT * FROM Users WHERE email='{}'", credentials.email.clone());
        let result = self.client.query(query).await;
        match result {
            Ok(mut result_set) => {
                ...
                match user {
                    Some(user) => {
                        let verify_password = verify_password(credentials.password.clone(), user.password.clone()).ok().unwrap();
                        dbg!(verify_password);
                        if verify_password {
                            let logged_in_query = format!("UPDATE Users SET logged_in = true WHERE email='{}'", user.email.clone());
                            match self.client.query(logged_in_query).await {
                                Ok(_) => Ok(Ok(EmailLoginInSuccess {
                                    email: user.email.clone(),
                                    username: user.username.clone()
                                })),
                                Err(err) => Err(err)
                            ...
                            Ok(Err("Password is incorrent try again".to_string()))
                        }
                    }
                    None => Ok(Err("Email is incorrent try again".to_string()))
                }
            }
            Err(err) => Err(err)
        }
    ...
    pub async fn delete_user(&self, username: String) -> Result<Result<Option<User>, String>, Error> {
        ...
        match get_user_result {
            ...
            Ok(None) => {
                Ok(Err("User not found".to_string()))
            }
            Err(err) => Err(err),
        }
    }
